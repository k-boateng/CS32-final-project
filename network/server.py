...
# key username : { socket: socket_obj, addr: (ip, port) }
clients = {}

def send_json(sock, data):
    #Sends a JSON‐encoded dict over the given socket."""
    try:
...
def broadcast(to_username, message_data):
    #Sends a chat message to the intended recipient if they're online.
...
def handle_client(client_socket, addr):
    ...
    try:
        ...
        # Acknowledges that the login is succesful
        send_json(client_socket, { "type": "login_ack", "status": "ok" })

        # 2) Main receiving loop for clients
        while True:
            raw = client_socket.recv(4096)
            if not raw:
                break
            ...
            if kind == "status_request":
                ...
                # Example is { "type":"message","from":"alice","to":"bob","content":"Hi!" }
                to_user = data.get("to")
